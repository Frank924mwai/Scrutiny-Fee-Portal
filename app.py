...
'    <span style="background-color: #C49A2A; color: #1E65B5; font-size: 0.72rem; font-weight: 700;'
...
# ── IMPROVEMENT 1: NaT-safe date formatter ─────────────────────────────────
# Prevents the literal string "NaT" from being written to Google Sheets when
...
def load_data() -> pd.DataFrame:
    try:
        # IMPROVEMENT 2: ttl=0 inside; the decorator's ttl=5 is the single cache controller.
        # The original ttl="5s" on conn.read() created a double-caching conflict where the
        # inner GSheetsConnection cache could serve stale data even after the outer cache expired.
        df = conn.read(ttl=0)
...
# IMPROVEMENT 3: Manual refresh button — allows force-refreshing data without
# enabling live mode. Useful after intake submissions to confirm the new record.
st.sidebar.markdown("---")
...
if current_page == "calculator":
    ...
    # ── IMPROVEMENT 4: Fee label disambiguation ────────────────────────────
    # Previously, "Scrutiny Fee" and "Total Fee" referred to the same number
    # whenever no add-ons were selected, causing confusion.
    #
    # Fix logic:
    #  - The per-department component is now explicitly labelled "Scrutiny Fee"
    #    or "Service Fee" (distinct from the combined total).
    #  - When add-ons are present: the total is labelled "Total Fee Payable"
    #    and the fee-note shows the breakdown, making clear it is NOT just the
    #    scrutiny fee.
    #  - When no add-ons: the single amount is unambiguously the
    #    "Total Scrutiny Fee Payable" / "Total Estates Fee Payable".
    ...
    if addon_accumulated > 0:
        ...
        invoice_fee_label = (
            "Total Scrutiny Fee Payable"
            if dept_choice == "Town Planning (Scrutiny)"
            else "Total Estates Fee Payable"
        )
    ...
    with col2:
        with st.container(border=True):
            ...
            if addon_accumulated > 0:
                # IMPROVEMENT 4 (cont.): Use base_fee_label ("Scrutiny Fee") not
                # "Base Fee", so the component is never confused with the total.
                addon_html = (
...
# ==============================================================================
    ...
    def clear_intake_data():
        """Safely clear all intake form fields."""
        st.session_state["intake_app_id"] = ""
        st.session_state["intake_applicant"] = ""
        st.session_state["intake_plot"] = ""
        st.session_state["intake_fee"] = 50000.0
        # Clear bundle checkboxes
        for key in ["inc_app", "inc_site", "inc_septic", "inc_sewer", "inc_parking"]:
            if key in st.session_state:
                st.session_state[key] = False

    # SUCCESS BANNER + CLEAR (must be before widgets)
    if st.session_state.get("intake_success_msg"):
        msg = st.session_state.pop("intake_success_msg")
        st.success(msg)
        st.balloons()
        clear_intake_data()   # Clear before re-rendering the form

    intake_dept = st.radio("Select Department", 
                          ["Town Planning (Scrutiny)", "Estates Services"], 
    ...
    with col1:
        app_id = st.text_input("Application / File ID", 
                              placeholder="e.g., BCC/TP/2026/250", 
                              key="intake_app_id")
        applicant_name = st.text_input("Applicant Name / Developer Entity", 
                                      key="intake_applicant")
        date_rcvd = st.date_input("Date Received", 
    ...
    with col2:
        plot_number = st.text_input("Plot Number / Parcel ID", 
                                   key="intake_plot")
        intake_category = st.selectbox("Category", 
                                      list(target_dict.keys()), 
                                      key="intake_cat")
        intake_subcategory = st.selectbox("Development Type", 
                                         list(target_dict[intake_category].keys()), 
                                         key="intake_sub")
    ...
    with st.container(border=True):
        ...
        input_fee_paid = st.number_input(intake_input_label, 
                                       min_value=0.0, 
                                       step=5000.0, 
                                       key="intake_fee")
        ...
        with bundle_col1:
            intake_inc_app = st.checkbox("Base App Fee (MK 15,000)", 
                                       value=True, disabled=not is_tp, key="inc_app")
            intake_inc_site = st.checkbox("Site Plan Cert. (MK 15,000)", 
                                        value=False, disabled=not is_tp, key="inc_site")
        with bundle_col2:
            intake_inc_septic = st.checkbox("Septic Tank (MK 40,000)", 
                                          value=False, disabled=not is_tp, key="inc_septic")
            intake_inc_sewer = st.checkbox("Sewer App Fee (MK 100,000)", 
                                         value=False, disabled=not is_tp, key="inc_sewer")
        with bundle_col3:
            intake_inc_parking = st.checkbox("Car Parking (MK 280,000)", 
                                           value=False, disabled=not is_tp, key="inc_parking")

        # Calculate deductions
        deductions = 0.0
        if is_tp:
            if intake_inc_app:
                deductions += BCC_RATES["Advertising"]["Application Fee"]["rate"]
            if intake_inc_septic:
                deductions += BCC_RATES["Septic Tank"]["Septic Tank Installation"]["rate"]
            if intake_inc_site:
                deductions += BCC_RATES["Miscellaneous"]["Site Plan Certification"]["rate"]
            if intake_inc_parking:
                deductions += BCC_RATES["Miscellaneous"]["One surface car parking space"]["rate"]
            if intake_inc_sewer:
                deductions += BCC_RATES["Miscellaneous"]["Sewer Application Fees"]["rate"]

        # Live net fee preview
        ...
        if submit_btn:
            errors = []
            if not app_id.strip():
                errors.append("Application ID Reference Number is required.")
            if not applicant_name.strip():
                errors.append("Applicant Name is required.")
            if not plot_number.strip():
            ...
            if errors:
                ...
                try:
                    ...
                    st.rerun()   # No direct clear here — clearing happens at the top on rerun
...
# ==============================================================================
    ...
    view_filter = st.radio(
        "View Data For:",
        ["All Departments", "Town Planning (Scrutiny)", "Estates Services"],
        horizontal=True
    )
    ...
    if df_filtered.empty:
        ...
        styled_df = df_display_sorted.style.set_properties(**{
...
# ==============================================================================
    ...
    st.markdown("<hr class='bcc-divider'>", unsafe_allow_html=True)
    search_query = st.text_input(
        "  🔍   Enter Application ID or Plot Number to load record:",
        placeholder="e.g., BCC/TP/2026/250"
    )
        ...
        if len(match_idx) == 0:
            ...
            if track_type == "Lease Application":
                ...
                steps = [
                    ...
                    "Signing by Director of Financial Services", "Initial CEO Signature",
                    "Document Preparation", "Final CEO Signature",
                ]

            saved_steps_str = str(record.get("Completed Steps", ""))
            saved_steps = (
                saved_steps_str.split(",")
                if saved_steps_str and saved_steps_str not in ("nan", "N/A")
                else []
            )
            ...
            with st.form("tracker_form"):
                ...
                if submitted:
                    ...
                    fresh_match_idx = df_fresh[
                        df_fresh["Application ID"].astype(str).str.upper() == str(app_id_target).upper()
                    ].index

                    if len(fresh_match_idx) > 0:
                        df_fresh.at[fresh_match_idx[0], "Completed Steps"] = new_steps_str

                        # IMPROVEMENT 10: Safe date normalisation before tracker write
                        df_fresh["Date Received"] = _fmt_date_col(df_fresh["Date Received"])

                        # IMPROVEMENT 11: Error handling around tracker conn.update()
                        try:
                        ...
                        st.error(
                            "  ❌   Sync Error: The original application record could not be found "
                            "in the database. It may have been deleted by another user. Please refresh the page."
                        )
