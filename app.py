...
<style>
    ...
    /* Target radio button option text specifically */
...
# ── Authentication Gate Layer (using secrets) ─────────────────────────────────
        ...
        with st.container(border=True):
            ...
            if submit_auth:
                try:
                    ...
                    correct_password = "GIS@!2026"  # Fallback for local development only
        ...
        with st.container(border=True):
            ...
            if unit_type == "sqm":
                ...
                if input_method == "Enter Total Area Manually":
                    input_val = st.number_input("Total Built-up Area (Square Meters)", 
                                              min_value=0.0, value=100.0, step=10.0, key="calc_man_sqm")
                else:
                    shape = st.selectbox("Select Shape Profile:", 
                                       ["Rectangle", "Triangle", "Trapezium", "Circle", "Semicircle"], 
                                       key="calc_shape")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if df_bcc.empty:
        ...
        if df_chart.empty:
            st.warning("⚠️ No valid date stamps found.")
        else:
            ...
            with col1:
                ...
                fig_trend.update_layout(
                    ...
                    font=dict(color="#3A4557", size=13),
                    title_font=dict(size=18, color="#1B2A4A"),
                    legend=dict(
                        orientation="h",
